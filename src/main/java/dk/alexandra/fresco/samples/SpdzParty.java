...
public class SpdzParty {
  ...
  public CloseableNetwork getNetwork() {
    return new SocketNetwork(netConf);
  }
  ...
  public static class Builder {
    ...
    public SpdzParty build() {
      ...
      BigInteger suitableModulus = ModulusFinder.findSuitableModulus(modLength);
      SpdzDummyDataSupplier supplier = new SpdzDummyDataSupplier(netConf.getMyId(),
          netConf.noOfParties(), new BigIntegerFieldDefinition(suitableModulus),
      ...
      SpdzResourcePool rp = new SpdzResourcePoolImpl(netConf.getMyId(), netConf.noOfParties(),
          new OpenedValueStoreImpl<>(), supplier, AesCtrDrbg::new);
...
//          getSeedOts(netConf.getMyId(), partyIds, PRG_SEED_LENGTH, drbg, new SocketNetwork(netConf));
